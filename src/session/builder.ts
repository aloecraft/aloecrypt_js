...
export class PartyCHALLENGE extends PluginModel {
    ...
    constructor(fields: { encrypted_challenge: Uint8Array; encrypted_check: Uint8Array }) {
        super();
...
export class PartyINTRO extends PluginModel {
    ...
    static fromMsgpack(data: Record<string, unknown>): PartyINTRO {
        return new PartyINTRO({
            ...
            stable_kem: KyberPublicKEM.fromMsgpack(data.stable_kem as Record<string, unknown>),
            session_kem: KyberPublicKEM.fromMsgpack(data.session_kem as Record<string, unknown>),
            verifier: DilithiumVerifier.fromMsgpack(data.verifier as Record<string, unknown>),
        });
    }
}
...
export class SessionBuilder extends PluginModel {
    ...
    static fromMsgpack(data: Record<string, unknown>): SessionBuilder {
        return new SessionBuilder({
            delegate_signer: DilithiumSigner.fromMsgpack(data.delegate_signer as Record<string, unknown>),
            stable_kem: KyberFullKEM.fromMsgpack(data.stable_kem as Record<string, unknown>),
            session_kem: KyberFullKEM.fromMsgpack(data.session_kem as Record<string, unknown>),
            nonce: data.nonce as Uint8Array,
            ...
            cipher: data.cipher ? FullCIPHER.fromMsgpack(data.cipher as Record<string, unknown>) : null,
            counterparty_intro: data.counterparty_intro ? PartyINTRO.fromMsgpack(data.counterparty_intro as Record<string, unknown>) : null,
            counterparty_cipher: data.counterparty_cipher ? PartySecret.fromMsgpack(data.counterparty_cipher as Record<string, unknown>) : null,
            counterparty_challenge: data.counterparty_challenge ? PartyChallenge.fromMsgpack(data.counterparty_challenge as Record<string, unknown>) : null,
    ...
    get address(): Uint8Array {
        return (async () => await call("session_builder_address", pack(this)))() as any;
    }
    ...
    async onCounterpartyIntro(intro: PartyINTRO): Promise<SessionBuilder> {
        const result = await call("session_builder_on_counterparty_intro", pack(this, intro));
        return SessionBuilder.fromMsgpack(unpack(result));
    }
    ...
    async onCounterpartyCipher(cipher: PartyCIPHER): Promise<SessionBuilder> {
        const result = await call("session_builder_on_counterparty_cipher", pack(this, cipher));
        return SessionBuilder.fromMsgpack(unpack(result));
    }

    async makePartyChallenge(): Promise<PartyCHALLENGE> {
        const result = await call("session_builder_make_party_challenge", pack(this));
        return PartyCHALLENGE.fromMsgpack(unpack(result));
    }

    async onCounterpartyChallenge(challenge: PartyCHALLENGE): Promise<SessionBuilder> {
        const result = await call("session_builder_on_counterparty_challenge", pack(this, challenge));
        return SessionBuilder.fromMsgpack(unpack(result));
    }

    async makePartyChallengeResponse(): Promise<PartyRESPONSE> {
        const result = await call("session_builder_make_party_challenge_response", pack(this));
        return PartyRESPONSE.fromMsgpack(unpack(result));
    }

    async onCounterpartyChallengeResponse(response: PartyRESPONSE): Promise<SessionBuilder> {
        const result = await call("session_builder_on_counterparty_challenge_response", pack(this, response));
        return SessionBuilder.fromMsgpack(unpack(result));
    }

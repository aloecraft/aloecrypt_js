...
export class KyberFullKEM extends PluginModel {
    ...
    static async fromXPem(pem: string, password: Uint8Array, salt: Uint8Array): Promise<KyberFullKEM> {
        const result = await call("kyber_kem_from_x_pem", pack(pem, password, salt));
        return KyberFullKEM.fromMsgpack(unpack(result));
    }
...
export class KyberPublicKEM extends PluginModel {
    ...
    static async fromPem(pem: string): Promise<KyberPublicKEM> {
        const result = await call("kyber_public_kem_from_pem", new TextEncoder().encode(pem));
        return KyberPublicKEM.fromMsgpack(unpack(result));
    }
    ...
    async encapsulate(): Promise<[Uint8Array, Uint8Array]> {
        const result = unpack(await call("kyber_public_kem_encapsulate", pack(this)));
